...
- **注意**：此模块不继承 spring-boot-starter-parent，仅使用纯 Spring 框架
- 多模块结构，包含 4 个子主题：
  - `spring-resource`：资源访问
  - `spring-metadata`：元数据读取
  - `spring-validator`：数据验证
  - `spring-convert`：类型转换（进一步细分为 6 个子模块）
...
cd sample03-spring-reading/spring-resource
mvn spring-boot:run

# spring-convert 的子模块示例
cd sample03-spring-reading/spring-convert/converter
mvn spring-boot:run
```
...
### Spring Boot 应用模块（sample01, sample02, sample04）
```
sampleXX-{name}/
├── src/
│   ├── main/
...
├── spring-resource/                # 资源访问示例
├── spring-metadata/                # 元数据示例
├── spring-validator/               # 验证器示例
└── spring-convert/                 # 类型转换示例（多模块）
    ├── property-editor/
    ├── converter/
    ├── converter-factory/
    ├── generic-converter/
...
- 使用纯 Spring 框架（spring-framework-core）
